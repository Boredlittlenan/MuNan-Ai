...
- 配置备份：设置页右上角支持导出/导入设置 JSON，便于迁移和备份。
...
- 设置页导出的配置 JSON 会包含 API Key、SecretId、SecretKey 等敏感字段，只适合个人备份。
...
- 导出当前表单配置为 JSON 备份文件。
- 从 JSON 备份文件导入配置到表单，确认后再点击“保存设置”写入配置文件。
- 编辑 ASR/TTS 配置。
...
- 导出设置：读取当前表单中的 `AppConfig`，生成 `munan-ai-settings-*.json` 下载文件。
- 导入设置：用户选择 JSON 文件后，前端解析并通过 `normalizeAppConfig()` 填回表单。
- 导入不会立刻写入磁盘，用户需要点击“保存设置”才会调用 `save_app_config`。
- 导出的文件可能包含密钥，不能提交到 Git，也不要发给无关人员。
