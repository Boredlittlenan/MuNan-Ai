...
- `AgentScheduledTask`：计划任务结构，包含任务标题、提示词、下一次执行时间、任务类型、`schedule_mode` 调度方式、对应的时间/星期/月日/间隔字段、执行模型、来源、状态和上次执行信息。`schedule_type` 与 `recurrence` 仍保留为兼容字段，但新逻辑以 `schedule_mode` 为准。
...
- `<scheduled_task>`：请求创建计划任务，包含 `title`、`schedule_mode`、`kind`、`prompt`。`kind` 只能是 `reminder` 或 `ai_prompt`。`schedule_mode=once` 需要明确 ISO `scheduled_at`；`daily` 需要 `time_of_day`；`weekly` 需要 `weekdays` 和 `time_of_day`；`monthly` 需要 `month_day` 和 `time_of_day`；`yearly` 需要 `year_month`、`year_month_day` 和 `time_of_day`；`custom_days` 需要 `custom_interval_days` 和 `time_of_day`。
...
- `agentScheduledTasks`：计划任务列表，由设置页、聊天页和全局运行器共同读写；应用打开时 `ScheduledTaskRunner` 负责到点执行。`scheduled_at` 表示下一次执行时间，非单次 `schedule_mode` 每次成功执行后会更新这个字段。
...
当前 Agent 已有设置、能力预览和聊天页低风险快速动作。优先看：
...
- `src/settings/ScheduledTaskSettingsPanel.tsx` 的计划任务表单、任务列表 UI 和手动编辑入口。
- `src/App.tsx` 的 `runAgentScheduledTaskCreation()`。
- `src/scheduledTasks/chatScheduledTasks.ts` 的 `createScheduledTasksFromPlan()`、`applyScheduledTaskPlan()`、`createScheduledTasksFromReply()`、`createScheduledTasksFromUserRequest()`。
