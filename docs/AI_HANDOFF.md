...
- `ModelType`：模型供应商联合类型。
- `Message`、`Conversation`：聊天消息与会话类型。
...
- 发送聊天消息：调用 `invoke<string>("chat_with_ai", { model, messages })`。
- 每条 AI 回复提供复制和朗读按钮。
...
点击输入框旁麦克风
  ...
  -> recordedBlobToWavBase64()
  -> Web Audio API 解码 Blob
  -> encodeAudioBufferToWav() 编码为 WAV
...
- 腾讯云识别引擎类型保存在 `speech.asr.tencent_engine_type`，默认 `16k_zh_en`，不会影响 OpenAI-like/MIMO 的 `speech.asr.model`。
...
- `base_url` 留空时默认使用 `https://asr.tencentcloudapi.com`。
...
- `speech.asr.tencent_engine_type`：腾讯云识别引擎类型，对应 `EngSerViceType`，默认 `16k_zh_en`。
