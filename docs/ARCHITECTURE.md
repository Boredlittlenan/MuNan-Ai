...
- `src/App.tsx`: renders AI HTML/CSS cards from `<ai_card>` blocks, supports simple/full card switching, and coordinates chat send/receive flow.
...
- `src/scheduledTasks/chatScheduledTasks.ts`: parses `<scheduled_task>` blocks, handles chat-side scheduled task creation/update/delete helpers, deterministic fallback parsing, and scheduled task notices.
