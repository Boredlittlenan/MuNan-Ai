...
- 计划任务：设置页提供独立“计划任务”选项，调度方式直接支持单次、每日、每周、每月、每年和自定义天数；不同方式展示对应的日期、星期、月份或时间字段。聊天中说“明早提醒我”或“以后每天 9 点告诉我天气”时，会先由专用规划器理解并创建真实任务，应用打开时由全局运行器到点执行，并写入对应模型的计划任务会话。
